...
int init_sensors(void) {
    ...
    for (size_t i = 0; i < BME68X_NUMOF; i++) {
        DEBUG("Initialize BME68X sensor %u ... ", i);
        ...
        if (dev->sensor.chip_id == BME68X_CHIP_ID) {
            ...
            do {
                ...
                for(int n=0; n < n_fields; n++) {
                    DEBUG(
                        "BME68X[%d].%d temp=%.2f, press=%.2f, hum=%.2f, gas=%.2f, status=0x%02x\n",
                        i, sample_count, data[n].temperature, data[n].pressure, data[n].humidity,
