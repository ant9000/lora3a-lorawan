...
def parse_record(data):
    Header   = collections.namedtuple('Header', 'bme68x_num bme68x_fp sps30_num senseair_num bsec_num is_compressed')
    Sensors  = collections.namedtuple('Sensors', 'vcc vpanel bme68x sps30 bsec senseair')
    ...
    Bsec     = collections.namedtuple('Bsec', 'iaq accuracy')
    ...
    header = Header(bme68x_num, bme68x_fp, sps30_num, senseair_num, bsec_num, is_compressed)
    if is_compressed:
    ...
    if bsec_num:
        fmt = "H" * bme68x_num
        iaq = struct.unpack_from(fmt, data, offset)
        offset += struct.calcsize(fmt)
        fmt = "B" * bme68x_num
        acc = struct.unpack_from(fmt, data, offset)
        offset += struct.calcsize(fmt)
        for i in range(bme68x_num):
            sensors.bsec.append(Bsec(iaq[i], acc[i]))
...
if __name__ == "__main__":
    ...
    for n, line in enumerate(fileinput.input()):
        ...
        try:
            ...
            print("BSEC:              ", output.header.bsec_num)
            ...
            for i, bsec in enumerate(output.sensors.bsec):
                print("[BSEC_{}] IAQ = {}, accuracy = {}".format(i, bsec.iaq, bsec.accuracy))
            print("")
